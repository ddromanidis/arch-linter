// Command arch-lint-vet is arch-lint as a vet tool:
//
//	go vet -vettool=$(which arch-lint-vet) ./...
//
// A separate binary from arch-lint because the two answer to different masters. `go vet`
// expects a unitchecker — it drives the build itself, hands over one package at a time as
// a JSON export file, and owns the output format and the exit code. The arch-lint command
