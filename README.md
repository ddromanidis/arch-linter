...
arch-lint checks imports too, and then checks something else.
...
Leaks hide in more places than a signature. arch-lint resolves real types, so it finds all
...
```sh
go install github.com/ddromanidis/arch-linter/cmd/arch-lint@latest
```

## Use

```sh
arch-lint init -preset hexagonal   # write a starting arch.yaml
arch-lint                          # check ./...
arch-lint diagram                  # print the component graph as Mermaid
```
...
```sh
arch-lint baseline      # writes .arch-baseline.yaml
```
...
something there were nine of still fails. Fix some, re-run `arch-lint baseline`, and the
numbers come down.

**Or waive one:**

```go
//arch-lint:ignore exports the migration tool is handed the raw handle on purpose
...
```sh
arch-lint -format github ./...
```

**go vet:**

```sh
go install github.com/ddromanidis/arch-linter/cmd/arch-lint-vet@latest
go vet -vettool=$(which arch-lint-vet) ./...
```
...
distributing it to every developer and CI runner. If that is not worth it, run `arch-lint`
...
```sh
arch-lint diagram >> README.md
```

Dotted edges mark a dependency a component holds privately — imported, not re-exported.

This is arch-lint's own architecture, generated by the command above and enforced by its
own CI:
...
need, they are mature and worth using. arch-lint exists for the other half.
